...
def test_update_image(
    ...
    new_filename = random_lower_string(70)
    new_identifier = random_lower_string(70)
    data["filename"] = new_filename
    data["identifier"] = new_identifier
    data['id'] = requested_id

    response = client.post(
        f"{settings.API_V1_STR}/image/update_image", json=data
    )
...
def test_update_image_error_not_found(
        client: TestClient, db: Session
) -> None:
    '''
