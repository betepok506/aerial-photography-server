...
# @pytest.mark.asyncio
def test_create_platform_name_sentinel(db: Session) -> None:
...
# @pytest.mark.asyncio
def test_get_platform_name_sentinel(db: Session) -> None:
    platform_name = random_lower_string()
    platform_name_in = PlatformNameSentinelCreate(name=platform_name)
    platform = crud.platform_name_sentinel.create(db=db, obj_in=platform_name_in)
    stored_platform = crud.platform_name_sentinel.get(db=db, id=platform.id)
...
# @pytest.mark.asyncio
def test_update_platform_name_sentinel(db: Session) -> None:
    platform_name = random_lower_string()
    platform_name_in = PlatformNameSentinelCreate(name=platform_name)
    platform = crud.platform_name_sentinel.create(db=db, obj_in=platform_name_in)

    new_platform = random_lower_string()
    item_update = PlatformNameSentinelUpdate(id=platform.id, name=new_platform)
    updated_platform = crud.platform_name_sentinel.update(db=db, db_obj=platform, obj_in=item_update)
    assert platform.id == updated_platform.id
    assert updated_platform.name == new_platform


# @pytest.mark.asyncio
def test_delete_platform_name_sentinel(db: Session) -> None:
    platform_name = random_lower_string()
    platform_name_in = PlatformNameSentinelCreate(name=platform_name)
    platform = crud.platform_name_sentinel.create(db=db, obj_in=platform_name_in)

    removed_platform = crud.platform_name_sentinel.remove(db=db, id=platform.id)
    stored_platform = crud.platform_name_sentinel.get(db=db, id=platform.id)
